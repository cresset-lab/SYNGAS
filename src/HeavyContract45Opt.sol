...
contract HeavyContract45Opt {
    ...
    function transfer(address to, uint256 value) public returns (bool) {
        ...
        uint256 valuePerIteration = value / MAX_ITERATIONS;
        for (uint256 i = 0; i < MAX_ITERATIONS; ++i) {
            dynamicArray.push(i);
            balanceOf[to] += valuePerIteration;
        }
    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        ...
        for (uint256 i = 0; i < MAX_ITERATIONS; ++i) {
            balanceOf[to] += complexCalculation(value, i);
        }
    ...
    function complexCalculation(uint256 base, uint256 multiplier) internal pure returns (uint256) {
        uint256 result = base;
        uint256 multiplierPlusOne = multiplier + 1;
        for (uint256 i = 0; i < multiplier; ++i) {
            result = (result * base) / multiplierPlusOne;
        }
        return result;
    }
