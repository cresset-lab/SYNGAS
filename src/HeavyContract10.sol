...
contract HeavyContract10 {
    ...
    function complexCalculation() public returns (uint256) {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            for (uint256 j = i; j < data.length; j++) {
                sum += data[j] * indexData[i];
                indexData[i] = sum; // Frequent writes to storage
            }
        }
    ...
    function heavyComputation(uint256 n) public returns (uint256) {
        ...
        for (uint256 i = 0; i < n; i++) {
            for (uint256 j = i; j < n; j++) {
                result += j % 3 == 0 ? j * 2 : j;
                data[i % data.length] = result; // Update storage based on computation
            }
        }
    ...
    function addDataNumbers(uint256 limit) public returns (uint256) {
        uint256 sum = 0;
        uint256 len = data.length <= limit ? data.length : limit;
