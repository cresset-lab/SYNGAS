...
contract HeavyContract53 {
    ...
    function calculateFactorial(uint256 n) public returns (uint256) {
        if (factorialResults[n] != 0) {
            return factorialResults[n];
        }
        if (n == 0 || n == 1) {
            return 1;
        } else {
            uint256 result = (n * calculateFactorial(n - 1)) % MODULUS;
    ...
    function computeFibonacci(uint256 n) public returns (uint256) {
        while (fibonacciResults.length <= n) {
            uint256 nextFib = (fibonacciResults[fibonacciResults.length - 1] + fibonacciResults[fibonacciResults.length - 2]) % MODULUS;
            fibonacciResults.push(nextFib);
        }
    ...
    function power(uint256 base, uint256 exp) internal pure returns (uint256) {
        ...
        if (exp % 2 == 0) {
            ...
            return (halfSquared * base) % MODULUS;
        }
    }
