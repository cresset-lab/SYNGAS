...
contract HeavyContract4Opt {
    ...
    function computeHeavySum(uint256 iterations) public {
        uint256 localTotalSum = totalSum;
        uint256 dataStorageLen = dataStorage.length;
        for (uint256 i = 0; i < iterations; i++) {
            uint256 increment = i + 1;
            for (uint256 j = 0; j < dataStorageLen; j++) {
                localTotalSum += dataStorage[j] * increment;
                dataMap[j] = localTotalSum;
            }
        }
        totalSum = localTotalSum;
    }
    ...
    function computeHeavyProduct(uint256 iterations) public {
        uint256 localTotalProduct = totalProduct;
        uint256 dataStorageLen = dataStorage.length;
        for (uint256 i = 1; i <= iterations; i++) {
            uint256 increment = i + 1;
            for (uint256 j = 0; j < dataStorageLen; j++) {
                localTotalProduct *= (dataStorage[j] + i) % 1000 + 1;
                dataMap[j] = localTotalProduct;
            }
        }
        totalProduct = localTotalProduct;
    }
