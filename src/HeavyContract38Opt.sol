...
contract HeavyContract38Opt {
    ...
    function complexCalculation(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 sum = 0;
        uint256 scaledA = a * FACTOR;
        for (uint8 i = 0; i < 20; i++) {
            for (uint8 j = 0; j < 15; j++) {
                sum += (scaledA + b) / FACTOR;
            }
        }
        return sum;
    }
    ...
    function searchAndModify(uint256 searchId) public {
        ...
        for (uint256 i = 0; i < userRecords.length; i++) {
            if (userRecords[i].id == searchId) {
                userRecords[i].value = performHeavyComputation(searchId, userRecords[i].value);
            }
        }
    }
